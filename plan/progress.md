...
- 更新时间：2026-09-22
- 当前阶段：Phase 1 身份认证与安全边界进行中；AUTH-09A 会话核心已通过 MySQL 服务级事务验收，登录/API 和 REST/IM 接入尚未完成
- 当前接口契约：revision `6`，contract version `4.0.0`，已随 AUTH-08B 提交 `16065b3` 和合并提交 `a1ddc65` 推送；会话核心不改变现有接口
...
- [x] 默认全量 92 项测试通过，2 项外部 MySQL 测试默认跳过；新增会话服务 MySQL 验收已显式通过，包含真实 JPA 与事务，用户存在性使用测试替身，完整登录链路仍待接入。
...
| Phase 2 测试与数据一致性 | 已有测试基础 | 待评估 | 92 项默认测试通过；已有基础 MySQL 验收，点赞一致性补偿尚未实现 |
...
- [>] `AUTH-09A` JPA 会话核心、9 项 H2 测试及 MySQL 服务级事务验收完成；持久化登录接入、公开刷新接口未完成。
- [ ] `AUTH-09B` 访问令牌会话关联、服务端退出接口及 REST/IM 会话撤销。
...
1. 接入持久化登录、JWT 会话 ID、三个服务和 IM 的撤销校验后再开放刷新/退出接口，并递增接口 revision。复用已完成的 MySQL 服务级验收，补充真实用户查询、JWT 签发失败回滚与故障关闭测试；不得只开放退出接口却保留旧 JWT 仅验签放行。
