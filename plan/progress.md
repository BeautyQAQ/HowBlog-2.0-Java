...
- 更新时间：2026-09-22
- 当前阶段：Phase 1 身份认证与安全边界进行中；AUTH-09A 会话核心已实现，登录/API 和 REST/IM 接入尚未完成
...
- [x] 默认全量 92 项测试通过（含 63 项 MVC、1 项角色 H2 JPA 和 9 项会话事务测试），1 项真实 MySQL 测试默认跳过；AUTH-08A 曾通过基础 SQL 验收，新会话服务仍未完成 MySQL 实际事务验收。
...
- [>] `AUTH-09A` JPA 会话核心与 9 项 H2 事务测试完成；持久化登录接入、公开刷新接口和 MySQL 服务级事务验收未完成。
...
1. 对 AUTH-09A 核心完成 MySQL 服务级事务与迁移映射验收；接入持久化登录、JWT 会话 ID、三个服务和 IM 的撤销校验后再开放刷新/退出接口，并递增接口 revision。不得只开放退出接口却保留旧 JWT 仅验签放行。
